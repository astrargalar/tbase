<?php
/**
 * Mi tema functions and definitions
 *
...
if ( ! function_exists( 'mitema_setup' ) ) :
	...
	function mitema_setup() {
		 ...
		 * If you're building a theme based on Mi tema, use a find and replace
...
function mitema_scripts() {
		...
		wp_enqueue_style('mitema_fonts', 'https://fonts.googleapis.com/css?family=Dosis:300|Roboto:400,400i,700,700i&display=swap');

	wp_enqueue_style( 'mitema-style', get_stylesheet_uri() );

	wp_enqueue_script( 'mitema-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'mitema-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	wp_enqueue_script( 'mitema-all.min', get_template_directory_uri() . '/js/all.min.js', array(), '20151215', true );
